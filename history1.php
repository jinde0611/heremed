...
<?php
  ...
  $query = "SELECT distinct f_name From file_upload WHERE f_name in (SELECT e_id FROM signup WHERE ido_location ='$search_name')";

  $query1 = "SELECT * From file_upload WHERE f_name in (SELECT e_id FROM signup WHERE ido_location ='$search_name')";
  //$query2 ="SELECT f_name , SUM(f_amount) FROM `file_upload` where f_name='$search_name' AND Verifiation_status ='1'";

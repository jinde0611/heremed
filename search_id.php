...
<?php
  ...
  $query = "SELECT distinct f_name From file_upload WHERE f_name in (SELECT e_id FROM signup WHERE emp_id ='$search_name')";

  $query1 = "SELECT * From file_upload WHERE f_name in (SELECT e_id FROM signup WHERE emp_id ='$search_name')";
	$query2 ="SELECT f_name , SUM(f_amount) FROM `file_upload` where f_name=(SELECT e_id FROM signup WHERE emp_id ='$search_name') AND Verifiation_status ='1' AND opd_type='Normal'";
   $query3 ="SELECT f_name , SUM(f_amount) FROM `file_upload` where f_name=(SELECT e_id FROM signup WHERE emp_id ='$search_name') AND Verifiation_status ='1' AND opd_type='Dental'";
  ...
  ?>
    <li>
           <div class="collapsible-header teal lighten-2">
             <i class="material-icons">portrait</i><?=  $row["f_name"]; ?> -- Total Amount Claimed ->
